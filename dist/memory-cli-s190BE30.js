...
import { F as loadConfig, lt as parseDurationMs, yn as splitShellArgs } from "./auth-profiles-B--FziTi.js";
...
import { r as getMemoryCommandSecretTargetIds, s as resolveCommandSecretRefsViaGateway } from "./command-secret-targets-xrO58BqY.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-ziYyg0F6.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-CvXBD-Wv.js").then((n) => n.t);
