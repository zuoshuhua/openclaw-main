import { F as getPrimaryCommand, G as isValueToken, I as getVerboseFlag, L as hasFlag, M as getCommandPositionalsWithRootOptions, N as getFlagValue, P as getPositiveIntFlagValue, R as hasHelpOrVersion, j as getCommandPathWithRootOptions } from "./globals-d3aR1MYC.js";
import "./paths-BMo6kTge.js";
import { d as defaultRuntime, r as enableConsoleCapture } from "./subsystem-kl-vrkYi.js";
...
import { Bn as loadDotEnv } from "./auth-profiles-B--FziTi.js";
...
import { i as formatUncaughtError } from "./errors-DrflaMHL.js";
...
import { h as installUnhandledRejectionHandler } from "./audio-transcription-runner-BBm1GGMT.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-DBXoDyX9.js";
import { t as assertSupportedRuntime } from "./runtime-guard-DCaKUJXu.js";
import { t as emitCliBanner } from "./banner-CNcg614A.js";
import "./doctor-config-flow-BOxZgVTr.js";
import { n as ensureConfigReady } from "./config-guard-DkEPzmIw.js";
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-CmC1MjtJ.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-ZVzifCvs.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-Ch0VEHJ8.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-s190BE30.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-HPZoiuE1.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-HPZoiuE1.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-CG5U8GMi.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-CG5U8GMi.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-CXCg7HUn.js");
	...
	if (primary) {
		const { getProgramContext } = await import("./program-context-Bdw9YypP.js").then((n) => n.n);
		const ctx = getProgramContext(program);
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-CyBH7Tn4.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis--h02cTHK.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { registerPluginCliCommands } = await import("./cli-DPmTxHvf.js");
		const { loadConfig } = await import("./auth-profiles-B--FziTi.js").then((n) => n.j);
