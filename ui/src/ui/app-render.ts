...
import { renderSkillCenter } from "./views/skill-center.ts";
        ...
        <div class="topbar-left">
          <div class="brand">
            <div class="brand-logo">
              <img src=${basePath ? `${basePath}/logo.png` : "/logo.png"} alt="算力作战" />
            </div>
        ...
        ${
          state.tab === "skills"
            ? renderSkillCenter({
                ...
                queryText: state.skillQueryText,
                edits: state.skillEdits,
                messages: state.skillMessages,
                busyKey: state.skillsBusyKey,
                onFilterChange: (next) => (state.skillsFilter = next),
                onRefresh: () => loadSkills(state, { clearMessages: true }),
                onSkillSelect: (skill) => (state.selectedSkill = skill),
                onQueryChange: (text) => (state.skillQueryText = text),
                onQuerySubmit: () => {
                  // 执行Skill查询 - 这里可以调用agent接口
                  if (state.selectedSkill && state.skillQueryText.trim()) {
                    // TODO: 调用agent执行skill
                    console.log("执行Skill:", state.selectedSkill.name, "查询:", state.skillQueryText);
                  }
                },
        ...
        ${
          state.tab === "chat"
            ? renderChat({
                ...
                skillsFilter: state.skillsFilter,
                onSkillsFilterChange: (filter) => (state.skillsFilter = filter),
                onSkillSelect: (skill) => {
                  // 将Skill名称插入到输入框
                  const skillTag = `@${skill.name} `;
                  state.chatMessage = state.chatMessage + skillTag;
                },
                // Skill category collapse state
                defaultSkillsExpanded: state.defaultSkillsExpanded,
                mySkillsExpanded: state.mySkillsExpanded,
                onToggleDefaultSkills: () => (state.defaultSkillsExpanded = !state.defaultSkillsExpanded),
                onToggleMySkills: () => (state.mySkillsExpanded = !state.mySkillsExpanded),
                // Skill refresh
                onSkillsRefresh: async () => {
                  state.skillsRefreshing = true;
                  // 先清空当前技能列表，强制重新渲染
                  ...
                  if (sessionKey) {
                    ...
                    if (parsed?.agentId) {
                      // 强制刷新，重新扫描技能目录
                      await loadAgentSkills(state as unknown as import("./app.js").OpenClawApp, parsed.agentId, true);
                    }
                  }
                  state.skillsRefreshing = false;
                },
                skillsRefreshing: state.skillsRefreshing,
              })
            : nothing
        }
